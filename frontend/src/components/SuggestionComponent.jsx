...
function SuggestionComponent() {
  ...
  courseFavorie.forEach((course) => {
    if (!suggestedCategories.includes(course.category)) {
      suggestedCategories.push(course.category);
    }
  });
  console.log("category", suggestedCategories);

  const suggestedCourses = mockCourses.filter(
    (course) =>
      suggestedCategories.includes(course.category) &&
      String(course._id) !== String(id), // loại bỏ khóa học đang xem
  );
  console.log("course", suggestedCourses);
            ...
            <div class="cursor-pointer text-center text-gray-500 dark:text-gray-400">
              ...
              <ul class="mt-4 flex justify-center space-x-4">
                ...
                <li>
                  <a
                    href="#"
                    class="text-gray-900 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white"
                  >
                    <svg
                      class="h-6 w-6"
                      fill="currentColor"
